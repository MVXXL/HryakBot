...
class Item:
    ...
    def get_data(item_id: str, key: str):
        if Item.clean_id(item_id) not in utils_config.items or key not in utils_config.items[Item.clean_id(item_id)]:
            return
        res = utils_config.items[Item.clean_id(item_id)][key]
    ...
    @staticmethod
    def exists(item_id):
        return Item.clean_id(item_id) in utils_config.items
    ...
    def set_skin_config(item_id: str, new_config):
        utils_config.items[item_id]['skin_config'] = new_config
    ...
    async def get_image_path(item_id: str):
