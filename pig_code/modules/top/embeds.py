...
async def generate_top_embed(inter, lang: str, title: str, description: str, users_list: list,
    ...
    fields = []
    best_users_field = {'name': translate(Locale.Top.best_of_the_bests, lang),
                        'value': '',
                        'inline': False}
    for n, i in enumerate(users_list[:3]):
        best_users_field['value'] += await generate_line(n, i)
    fields.append(best_users_field)
    other_users_field = {'name': translate(Locale.Top.also_not_bad, lang),
                         'value': '',
                         'inline': False}
    for n, i in enumerate(users_list[3:]):
    ...
    if user_position is not None:
        if other_users_field['value']:
            other_users_field[
                'value'] += f"\n\n{translate(Locale.Top.your_position, lang, format_options={'place': user_position + 1})}"
