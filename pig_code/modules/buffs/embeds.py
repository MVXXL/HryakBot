...
async def generate_basic_buffs_embed(inter, lang, thumbnail_url=None):
    ...
    buffs = await Utils.get_all_pig_buffs(inter.client, inter.user.id)
    for buff in buffs:
        if Item.exists(buff):
...
async def generate_buffs_multipliers_embed(inter, lang, buff_type: str, thumbnail_url=None):
    ...
    calculated_buffs = await Utils.calculate_buff_multipliers(inter.client, inter.user.id, use_buffs=False)
