...
async def wardrobe(inter, message=None, select_item_component_id: str = 'inventory_item_select', pre_command_check=True, ephemeral=False, edit_original_message=True):
    ...
    await BotUtils.pagination(inter if message is None else message, lang,
                              embeds=await BotUtils.generate_list_embeds(inter, items_by_cats, lang, empty_desc,
                                                                         show_amount_in_field_value=True, select_item_component_id=select_item_component_id,
                                                                         title=Locales.Wardrobe.wardrobe_title[lang],
                                                                         basic_info=[['type'], ['rarity']]),
                              embed_thumbnail_file=embed_thumbnail_file, ephemeral=ephemeral, edit_original_message=edit_original_message)


async def inventory(inter, message=None, select_item_component_id: str = 'inventory_item_select', pre_command_check=True, ephemeral=False, edit_original_message=True):
    if pre_command_check:
        await BotUtils.pre_command_check(inter)
    lang = User.get_language(inter.author.id)
