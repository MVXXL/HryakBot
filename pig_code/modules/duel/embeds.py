...
def fight_is_starting(inter, lang, user: discord.User, time_to_start: int, chances) -> discord.Embed:
    embed = generate_embed(
        ...
        description=translate(Locales.Duel.fight_is_starting_desc, lang,
                              {'pig1': Pig.get_name(inter.user.id), 'pig2': Pig.get_name(user.id)}),
        ...
        fields=[{'name': f'{Pig.get_name(inter.user.id)}',
                 'value': translate(Locales.Duel.fight_starting_field_value, lang,
                                    {'weight': Pig.get_weight(inter.user.id), 'chance': chances[0]}),
                 'inline': True},
...
def user_won(inter, lang, user: discord.User, money_earned, gif) -> discord.Embed:
    embed = generate_embed(
        ...
        description=translate(Locales.Duel.fight_ended_desc, lang,
                              {'pig': Pig.get_name(user.id), 'user': user.mention, 'money_earned': money_earned}),
        thumbnail_url=gif,
