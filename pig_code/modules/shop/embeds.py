...
def shop_item_bought(inter, item_id, lang) -> disnake.Embed:
    embed = generate_embed(
        title=Locales.ShopItemBought.title[lang].format(item=Inventory.get_item_name(item_id, lang)),
        description=Locales.ShopItemBought.desc[lang],
...
def item_buy_cooldown(inter, item_id, lang) -> disnake.Embed:
    embed = generate_embed(
        ...
        description=Locales.ErrorCallbacks.shop_buy_cooldown_desc[lang].format(
            ...
            timestamp=Shop.get_timestamp_of_new_item(inter.author.id, item_id)),
        prefix=Func.generate_prefix('scd'),
        inter=inter,
    )
    return embed
